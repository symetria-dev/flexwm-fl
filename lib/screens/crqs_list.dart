...
class _CreditRequestList extends State<CreditRequestList> {
  ...
  Future<List<SoCreditRequest>> fetchSoCreditRequests() async {
    final response = await http.Client().get(Uri.parse(
        params.getAppUrl(params.instance) + 'restcrqs;' +
    ...
    return parsed.map<SoCreditRequest>((json) => SoCreditRequest.fromJson(json)).toList();
  }
  ...
  showAlertDialog(BuildContext context) {
    ...
    AlertDialog alert = AlertDialog(
      title: const Text("Si tienes dudas del proceso, por favor contáctanos al:"),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Row(
            children: const [
              Icon(Icons.mail, color: Colors.blueGrey,), SizedBox(width: 5,),
              Text("soporte@edupass.mx",style: TextStyle(color: Colors.grey),),
            ],
          ),
          Row(
            children: const [
              Icon(Icons.phone, color: Colors.blueGrey,), SizedBox(width: 5,),
              Text("0155 6387 1129",style: TextStyle(color: Colors.grey),)
            ],
          )
        ],
      ),
          ...
          children: [
            ...
            ListTile(
              onTap: () => Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (_) =>
                        CustDataForm(creditRequest:SoCreditRequest.empty())),
              ).then((value) => setState((){
                _futureSoCreditRequests = fetchSoCreditRequests();
              })),
              leading:
  ...
  Future<void> _pullRefresh() async {
    List<SoCreditRequest> freshFutureSoWFlowSteps = await fetchSoCreditRequests();
    setState(() {
